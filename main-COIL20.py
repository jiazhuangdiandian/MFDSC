...
class DSCNet(nn.Module):
    ...
    def get_C(self):
        ...
        C1 = self.self_expression1.Coefficient.detach().to('cpu').numpy()
        C2 = self.self_expression2.Coefficient.detach().to('cpu').numpy()
        C3 = self.self_expression3.Coefficient.detach().to('cpu').numpy()
...
def train(model,  # type: DSCNet
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % show == 0 or epoch == epochs - 1:
            ...
            # torch.save(model.state_dict(), 'H:/5/results\coil20/1\coil20-%d.pkl' % epoch)
            # torch.save(model.state_dict(), 'H:/5/results\coil20/2\coil20-%d.pkl' % epoch)
...
if __name__ == "__main__":
    ...
    if db == 'coil20':
        # load data
        data = sio.loadmat('dataset/COIL20.mat')
        ...
        # ae_state_dict = torch.load('D:/5\DSC\pretrained_weights_original/%s-151.pkl' % db)
        ae_state_dict = torch.load('D:/5\DSC\pretrained_weights_original/%s-147.pkl' % db)
