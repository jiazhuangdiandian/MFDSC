    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % show == 0 or epoch == epochs - 1:
            ...
            if acc(y, y_pred)>min_acc:
                ...
                torch.save(model.state_dict(), 'H:/5/results/mnist/mnist(0).pkl')
            # torch.save(model.state_dict(), 'H:/5/results\mnist/1\mnist-%d.pkl' % epoch)
            # torch.save(model.state_dict(), 'H:/5/results\mnist/2\mnist-%d.pkl' % epoch)
...
if __name__ == "__main__":
    ...
    if db == 'mnist':
        # load data
        data = sio.loadmat('dataset/mnist1000.mat')
        ...
        alpha = 0.04 # threshold of C 0.4-->0.04
