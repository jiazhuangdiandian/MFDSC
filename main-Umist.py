    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % show == 0 or epoch == epochs - 1:
            ...
            if acc(y, y_pred)>min_acc:
                ...
                torch.save(model.state_dict(), 'H:/5/results/umist/umist(0).pkl')
            # torch.save(model.state_dict(), 'H:/5/results/umist/1/umist-%d.pkl' % epoch)
            # torch.save(model.state_dict(), 'H:/5/results/umist/2/umist-%d.pkl' % epoch)
            # model.save_metrics_to_csv(os.path.join('H:/5/results/metrics', '%s.csv' % args.db), metrics)
            # model.save_metrics_to_csv(os.path.join('H:/5/results/metrics', '%s1.csv' % args.db), metrics)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--db', default='Umist',
                        choices=['coil20','orl','Umist'])
    ...
    if db == 'Umist':
        # load data
        data = sio.loadmat('dataset/Umist_480n_1024d.mat')
        # print(data.get('gnd').shape)
        x, y = data['img'].reshape((-1, 1, 32, 32)), data['label']
        ...
        alpha = 0.08 # threshold of C 0.08
        dim_subspace = 8  # dimension of each subspace
        ro = 8  #
        ...
        # ae_state_dict = torch.load('D:/5\DSC\pretrained_weights_original/%s-502.pkl' % db)
        # ae_state_dict = torch.load('H:/5/results/umist/umist(0).pkl')
